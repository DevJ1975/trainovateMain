  ...
  return (
    <>
      ...
      <div className="pt-32 md:pt-40 pb-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-7">
              ...
              <p className="tnv-body mt-8 max-w-xl text-pretty">
                Trainovate.ai is a Service-Disabled Veteran-Owned Small Business
                shipping a workforce-safety platform for DoD, VA, and federally
                regulated industry. Soteria is our product line; the rest of
                this page is the credentialing.
              </p>
        ...
        <section className="tnv-container tnv-section mt-24">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-bone/8 border border-bone/8">
            {[
              ...
              <div key={r.label} className="bg-ink/40 p-6 md:p-8">
        ...
        <section className="tnv-container tnv-section mt-32">
          ...
          <div className="mt-12 border border-bone/8">
            {NAICS.map((n) => (
              <div
                key={n.code}
                className="flex items-baseline justify-between gap-6 px-6 py-5 border-b border-bone/8 last:border-b-0 bg-ink/30"
              >
                <div className="flex items-baseline gap-6">
                  <div className="font-mono text-2xl text-cobalt-soft">{n.code}</div>
                  <div className="text-bone/90 font-sans">{n.title}</div>
                </div>
                {n.primary && (
                  <span className="font-mono text-[10px] uppercase tracking-eyebrow text-cobalt-soft border border-cobalt/40 px-2 py-1">
                    Primary
                  </span>
                )}
              </div>
        ...
        <section className="tnv-container tnv-section mt-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-bone/8 border border-bone/8 mt-10">
            {[
              ...
              <div key={d.t} className="bg-ink/40 p-8">
                <div className="text-cobalt-soft font-mono text-[10px] uppercase tracking-eyebrow">→</div>
        ...
        {/* Vehicles */}
        <section className="tnv-container tnv-section mt-32">
          <Eyebrow index="05.C" label="CONTRACT VEHICLES" />
          <div className="mt-10 border border-bone/8">
            {vehicles.map((v) => (
              <div
                key={v.name}
                className="flex items-center justify-between gap-6 px-6 py-5 border-b border-bone/8 last:border-b-0 bg-ink/30"
              >
                ...
                <span
                  className={`font-mono text-[10px] uppercase tracking-eyebrow border px-2 py-1 ${
                    v.tone === "signal"
                      ? "text-cobalt-soft border-cobalt/40"
                      : "text-flare border-flare/40"
                  }`}
                >
                  {v.status}
                </span>
        ...
        <section className="tnv-container tnv-section mt-32">
          <div className="tnv-glass rounded-2xl p-10 md:p-16 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <Eyebrow label="TEAMING" />
              <h3 className="tnv-h3 mt-4 max-w-md">
                Open to set-aside teaming and capability-statement requests.
              </h3>
            </div>
